...
class GCSim:
    ...
    def heatmap(self, x0, timepoints, max_conc, num_val, input_a, input_b, output_protein, title = '', 
        ...
        for a_ind, A_c in enumerate(np.linspace(0, max_conc, num_val)):
            for b_ind, B_c in enumerate(np.linspace(0, max_conc, num_val)):
                x0[input_a] = A_c
                x0[input_b] = B_c
                R = self.CRN.simulate_with_bioscrape_via_sbml(timepoints, initial_condition_dict = x0)
    ...
    def basicsim(self, x0, timepoints, protein, title = '', xlabel = '', ylabel = ''):
        
        plt.figure()
        
        R = self.CRN.simulate_with_bioscrape_via_sbml(timepoints, initial_condition_dict = x0)
        
        plt.plot(R['time'], R[f'{protein}'], label = f'{protein}')
        plt.legend()
        plt.title(title)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
