# -*- coding: utf-8 -*-
"""
Created on Tue Aug 12 20:09:50 2025
...
component_parameters = {
    ...
    ParameterKey(mechanism = 'simple_transcription', part_id = 'P_BAD_Ara_2x_AraC_2x', name = "ktx"): 0.05,
...
AmeR = Species('AmeR', material_type='protein')

HlyIIR = Species('HlyIIR', material_type='protein')
...
#                           parameters = component_parameters)
P_HlyIIR = RegulatedPromoter('P_HlyIIR',  regulators = [HlyIIR], leak=True, 
                          parameters = component_parameters)
P_AmeR = RegulatedPromoter('P_AmeR',  regulators = [AmeR], leak=True, 
                          parameters = component_parameters)
...
protein_lst = ['protein_BetI_degtagged']

#Plotting
for a in [0, 17.5]:
    for b in [0, 17.5]:

            x0 = {BetI_construct.get_species():1,
                  HlyIIR:a, AmeR:b}
            timepoints = np.linspace(0, 6000, 6000)
            R = sim.basicsim(x0, timepoints, protein_lst, title = f'HlyIIR = {a}, AmeR = {b}')
